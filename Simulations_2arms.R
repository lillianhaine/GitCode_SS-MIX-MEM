...
if(scenario == 1){ 
  w_ctrl = 1; w_trt = seq(0, 1, 0.1); Bias_ctrl = Bias_trt = 0 
  #####go across many different OR options: 
  logOR = 1; OR = exp(logOR)
  grid_options = data.frame(expand.grid(w_ctrl, w_trt, Bias_ctrl, Bias_trt, logOR))
  colnames(grid_options) = c("w_ctrl", "w_trt", "Bias_ctrl", "Bias_trt", "logOR")
  print(dim(grid_options))
}
if(scenario == 2){ 
  w_ctrl = 1; w_trt = seq(0, 1, 0.1); Bias_ctrl = c(-1, 0);  Bias_trt = seq(-4, 4, 1)
  #####go across many different OR options: 
  logOR = 1; OR = exp(logOR)
  grid_options = data.frame(expand.grid(w_ctrl, w_trt, Bias_ctrl, Bias_trt, logOR))
  colnames(grid_options) = c("w_ctrl", "w_trt", "Bias_ctrl", "Bias_trt", "logOR")
...
source(paste0(wd, "/Functions_2arms.R") )
...
#get_conf <- get_conf(pctrl, ptrt, w, 10000, num_covs)
######function to generate the data:##############################
###inputs:
######mix_prop: from 0 to 1 if 1 that means all are from component 1 or in the RCT
data_generation <- function(mix_prop_ctrl, mix_prop_trt, logOddsRatio, bias_ext_ctrl, bias_ext_trt){
  #####across all scenarios we have: 
  pctrl = 0.30; num_covs = 5; n_trial = 200; n_ext_ctrl = 1000; n_ext_trt = 1000
  ext_dat_mean = c(0, -5)
  ...
  #####generate X matrices: #####
      ...
      components_ctrl <- sample(1:2,prob=c(mix_prop_ctrl, 1-mix_prop_ctrl),size=n_ext_ctrl,replace=TRUE)
      X_ext_ctrl <- do.call(rbind, lapply(components_ctrl, 
                                          function(x){
                                            mvrnorm(n=1,
                                                    mu=rep(ext_dat_mean[x], num_covs),
                                                    Sigma) } ) ) 
      components_trt <- sample(1:2,prob=c(mix_prop_trt, 1-mix_prop_trt),size=n_ext_trt,replace=TRUE)
      X_ext_trt <- do.call(rbind, lapply(components_trt, 
                                          function(x){
                                            mvrnorm(n=1,
                                                    mu=rep(ext_dat_mean[x], num_covs),
                                                    Sigma) } ) ) 
  ...
  #####generate outcomes: #####
    ...
    ########external RWD outcomes: #####
      Y_ext_ctrl <- rbinom(n_ext_ctrl, 1, prob = inv.logit(beta0_tau$beta0+X_ext_ctrl%*%beta+bias_ext_ctrl) )
      Y_ext_trt <- rbinom(n_ext_trt, 1, prob = inv.logit(beta0_tau$beta0+beta0_tau$tau+X_ext_trt%*%beta+bias_ext_trt) )
      
  #####generate propensity scores: #####
      ps_trial_dat <- data.frame(cbind( data.frame(Y = c(Y_ctrl, Y_trt), 
                                                   Trt = c( rep(0, n_trial/2), rep(1, n_trial/2)), 
                                                   Trial = 1), X_trial) )
      ps_ext_dat <- data.frame(cbind( data.frame(Y = c(Y_ext_ctrl, Y_ext_trt), 
                                                   Trt = c( rep(0, n_ext_ctrl), rep(1, n_ext_trt) ), 
                                                   Trial = 0), rbind(X_ext_ctrl, X_ext_trt) ) )
  ...
  #################### a data frame with the following named columns: Y, PS, logitPS, Trt
      ...
      trial <- data.frame(Y = c(Y_ctrl, Y_trt), 
                          PS = PS_trial, 
                          Trt = c( rep(0, n_trial/2), rep(1, n_trial/2))) %>% mutate(logitPS = logit(PS))
      ext <- data.frame(Y = c(Y_ext_ctrl, Y_ext_trt), 
                          PS = PS_ext, 
                          Trt = c(rep(0, n_ext_ctrl), rep(1, n_ext_trt)), 
                        trueZ = c(components_ctrl, components_trt) ) %>% mutate(logitPS = logit(PS)) 
      dat_gen = list()
...
########################################################################
analyse_sims <- function(parameters, sim_results, NB_results, n_rct_ctrl, n_rct_trt){
  n_rct = n_rct_ctrl+n_rct_trt
  ...
  ESSS_logOR <- ESSS(NB_results$logOR$var, var_logOR, n_rct)
  ...
  ESSS_pt <- ESSS(NB_results$pt$var, var_pt, n_rct_trt)
...
pooled_OR <- function(trt_dat, ctrl_dat, obs_dat_trt, obs_dat_ctrl){
  y_trt_all <- c(trt_dat$Y, obs_dat_trt$Y); 
  y_ctrl_all <- c(ctrl_dat$Y, obs_dat_ctrl$Y);
  pt_est <- mean(y_trt_all); pc_est = mean(y_ctrl_all); 
  
  log_OR_draws <- c()
  for(i in 1:10000){
    yt <- sum(y_trt_all); nt <- length(y_trt_all)
    pt <- rbeta(1, 1+yt, 1+nt-yt)
    yc <- sum(y_ctrl_all); nc <- length(y_ctrl_all)
    pc <- rbeta(1, 1+yc, 1+nc-yc)
    log_OR_draws[i] <- log_oddsratio(pc, pt) 
  }
  
  logOR_est <- mean(log_OR_draws[-c(1:1000)]);
  return(logOR_est)
}
########################################################################
########################################################################
...
########################################################################
run_sims <- function(nsim, method, parameters, scenario, job, n_rct_ctrl = 50, n_rct_trt = 50, wd, no_cores){
  ...
  cur_mix_trt = parameters$w_trt; cur_mix_ctrl = parameters$w_ctrl; 
  logOR_cur = parameters$logOR; bias_cur_trt = parameters$Bias_trt; bias_cur_ctrl = parameters$Bias_ctrl
  ...
  if(method == "NB"){
    ...
    dat_cur <- lapply(1:nsim, function(x) data_generation(cur_mix_ctrl, cur_mix_trt, logOR_cur, bias_cur_ctrl, bias_cur_trt))
    print(paste0("got to NB_run"))
    NB_res <- mclapply(1:nsim, function(x) NB(dat_cur[[x]], 10000, 1000), mc.cores = no_cores)
    ...
    saveRDS(NB_res, file = NB_name)
    print(paste0("got to analyse sims:"))
    print((NB_res[[1]]))
    print(NB_res[[nsim]])
    analysis = lapply(1:nsim, function(x) analyse_sims(trueparams, NB_res[[x]], NB_res[[x]], n_rct_ctrl, n_rct_trt)) 
    }
  if(method != "NB"){
    ...
    if(method == "SS_MIX_MEM_Capped"){res = mclapply(1:nsim, function(x) {
      SS_MIX_MEM(dat_cur[[x]], 3000, 4000, 3000, 1000, "capped")}, mc.cores = no_cores ) }
    if(method == "SS_MIX_MEM_FB"){res = mclapply(1:nsim, function(x) {
     SS_MIX_MEM(dat_cur[[x]], 3000, 4000, 3000, 1000, "FB")} , mc.cores = no_cores) }
    if(method == "MEM_Capped"){res = mclapply(1:nsim, function(x) {MEM(dat_cur[[x]], 10000, 1000, "capped") } , mc.cores = no_cores) }
    if(method == "MEM_FB"){res = mclapply(1:nsim, function(x) {MEM(dat_cur[[x]], 10000, 1000, "FB") } , mc.cores = no_cores) }
    print(paste0("got to analyse sims:"))
    print((res[[1]]))
    print(res[[nsim]])
    analysis = lapply(1:nsim, function(x) {analyse_sims(trueparams, res[[x]], NB_res[[x]], n_rct_ctrl, n_rct_trt)})
  }
  library(dplyr); library(tidyr)
  trt_dat = lapply(1:nsim, function(x) filter(dat_cur[[x]][[1]], Trt == 1))
  ctrl_dat = lapply(1:nsim, function(x) filter(dat_cur[[x]][[1]], Trt == 0))
  obs_dat_trt = lapply(1:nsim, function(x) filter(dat_cur[[x]][[2]], Trt == 1))
  obs_dat_ctrl = lapply(1:nsim, function(x) filter(dat_cur[[x]][[2]], Trt == 0))
  pooled_OR <- lapply(1:nsim, function(x) pooled_OR(trt_dat[[x]], ctrl_dat[[x]], obs_dat_trt[[x]], obs_dat_ctrl[[x]]))
  print(paste0("got to pcres:"))
  true_conf_logOR <- mean(sapply(1:nsim, function(x) pooled_OR[[x]] - trueparams$logOR))
  rel_conf_logOR <- mean(sapply(1:nsim, function(x) pooled_OR[[x]]/trueparams$logOR))
  ######get means of analysis results for each term: 
  pc_res =  colMeans( do.call(rbind, lapply(1:nsim, function(x) as.numeric (analysis[[x]]$pc) ))  )  
  pc_res = data.frame(bias = pc_res[1], var = pc_res[2], MSE = pc_res[3], ESSS = pc_res[4],  
                       method = method, 
                       metric = "pc", 
                       scenario = scenario,                       
                      true_w_ctrl = parameters$w_ctrl, 
                      true_w_trt= parameters$w_trt, 
                       true_logOR = logOR_cur, 
                       true_bias_ctrl = bias_cur_ctrl, 
                      true_bias_trt = bias_cur_trt, 
                      true_conf_logOR = true_conf_logOR, 
                      rel_conf_logoR = rel_conf_logOR)
  print(paste0("got to pt res:"))
  pt_res =  colMeans( do.call(rbind, lapply(1:nsim, function(x) as.numeric (analysis[[x]]$pt) ))  )  
  ...
  logOR_res =  colMeans( do.call(rbind, lapply(1:nsim, function(x) as.numeric (analysis[[x]]$logOR) ))  )  
  ...
  if(method %in% c("SS_MIX_MEM_Capped", "SS_MIX_MEM_FB") ){
    weights_ctrl = mean( do.call(rbind, lapply(1:nsim, function(x) as.numeric (res[[x]]$weights_ctrl) ))  ) 
    pip_ctrl = mean( do.call(rbind, lapply(1:nsim, function(x) as.numeric (res[[x]]$pip_ctrl) ))  ) 
    mix_ctrl = mean( do.call(rbind, lapply(1:nsim, function(x) as.numeric (res[[x]]$mix_param_ctrl) ))  ) 
    mem_ctrl = mean( do.call(rbind, lapply(1:nsim, function(x) as.numeric (res[[x]]$mem_ctrl) ))  ) 
    weights_trt = mean( do.call(rbind, lapply(1:nsim, function(x) as.numeric (res[[x]]$weights_trt) ))  ) 
    pip_trt = mean( do.call(rbind, lapply(1:nsim, function(x) as.numeric (res[[x]]$pip_trt) ))  ) 
    mix_trt = mean( do.call(rbind, lapply(1:nsim, function(x) as.numeric (res[[x]]$mix_param_trt) ))  ) 
    mem_trt = mean( do.call(rbind, lapply(1:nsim, function(x) as.numeric (res[[x]]$mem_trt) ))  ) 
    
    mix_mem = data.frame("mix_ctrl" = mix_ctrl, "mem_ctrl" = mem_ctrl, "weights_ctrl" = weights_ctrl, "pip_ctrl" = pip_ctrl,
                         "mix_trt" = mix_trt, "mem_trt" = mem_trt, "weights_trt" = weights_trt, "pip_trt" = pip_trt)
  ...
  if(method %in% c("MEM_Capped", "MEM_FB") ){
    ...
    mix_mem = data.frame("mem_ctrl" = mem_ctrl, "weights_ctrl" = weights_ctrl, "pip_ctrl" = pip_ctrl,
                         "mem_trt" = mem_trt, "weights_trt" = weights_trt, "pip_trt" = pip_trt)
    write.table(mix_mem, paste0(new_wd, "/", method,"/scen_", scenario, "job_", job, "_mix_mems_info_", method, ".txt" ) )}
...
run_sims(nsim, method, cur_params, scenario, job, n_rct_ctrl = 100, n_rct_trt = 100, wd, ncores)

# # ###test with 2 nsim: 
# nsim = 2; cur_params = grid_options[job, ]
# NB_check = run_sims(nsim, "NB", cur_params, 1, 1, 50, wd, 2)
# ####check all methods:
# methods.vect = c("MEM_Capped", "MEM_FB", "SS_MIX_MEM_Capped", "SS_MIX_MEM_FB")
# 
# system.time( {all_methods = lapply(1:length( methods.vect), function(x)
#   run_sims(nsim, methods.vect[x], 
#            cur_params, scenario, 
#            job, n_rct_ctrl = 100, n_rct_trt = 100, wd, ncores) )} )
