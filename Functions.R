########################################################################
########################################################################
##Functions used to run each method/analyze methods in sims:
########################################################################
########################################################################
##on my computer: 
#wd <- "~/Desktop/Dissertation/Paper1/Simulations_Code/Simulations_1Arm/"
...
MEM <- function(dat, M, nburnin, prior.text = "FB"){
  ...
  r_trt_ext = 0; n_trt_ext = 0

  ####get the prior values: 
  if(prior.text == "FB"){ pip_ctrl = 0.50; pip_trt = 0 } 
...
########################################################################
PSCL <- function(dat){
  library(psrwe, quietly = TRUE)
  ###NOTE: we already have the propensity score estimated: 
  ####step 1: put data in a form the psrwe functions will recognize ########
  #####columns currently included in the data frame for the trial is: PS, Y, Trt
  library(tidyr); library(dplyr)
  nct = nrow(dat[[1]]); nobs = nrow(dat[[2]])
  trial_data = dat[[1]] %>% mutate(Group = 0) 
  ext_data = dat[[2]] %>% mutate(Group = 1) %>% 
    select(-c("trueZ"))
  
  ######for dta_ps_rct to work we need to have the following: 
  data_pscl = data.frame(rbind(trial_data, ext_data))
  
  dta_ps_rct <- rwe_ps_est(data_pscl,
                           v_covs = "X1+X2+X3+X4+X5",
                           v_grp = "Group", cur_grp_level = 0,
                           v_arm = "Trt", ctl_arm_level = 0,
                           ps_method = "logistic", nstrata = 1)
  ####replace the ps estimated up here with the PS from the data since the PS were already estimated!
  # dat_new <- mutate(dta_ps_rct$data, OG_PS = PS, PSCL_PS = `_ps_`) %>% 
  #   mutate(`_ps_`= OG_PS)
  
  # dta_ps_rct$data = dat_new
  
  ###step 2: get the  discounting parameters: ##############
  n_ctrl = nrow(filter(trial_data, Trt == 0))
  disc_ps <- rwe_ps_borrow(dta_ps_rct, total_borrow = n_ctrl)
  
  ###step 3: apply the PSCL approach ################
  rst_cl_rct <- rwe_ps_compl(disc_ps,
                             outcome_type = "binary",
                             v_outcome = "Y")
  rst_cl_rct_ci <- rwe_ps_ci(rst_cl_rct)
  
  ###step 4: get the logOR, the pc, and the pt results ################
  
  ###########pc res ################
  pc_est = rst_cl_rct$Control$Overall_Estimate$Mean
  pc_var = (rst_cl_rct$Control$Overall_Estimate$StdErr)^2
  pc_ci = rst_cl_rct_ci$CI$Control$Overall_Estimate
  
  pc_res = list(est = pc_est, var = pc_var, ci = pc_ci)
  
  ###########pt res ################ 
  pt_est = rst_cl_rct$Treatment$Overall_Estimate$Mean
  pt_var = (rst_cl_rct$Treatment$Overall_Estimate$StdErr)^2
  pt_ci = rst_cl_rct_ci$CI$Treatment$Overall_Estimate
  
  pt_res = list(est = pt_est, var = pt_var, ci = pt_ci)
  
  ###########logOR res: uses delta method for variance: ################
  if(pc_est == 0){pc_est = NA}
  if(pc_var == 0){pc_var = NA}
  if(pt_est == 0){pt_est = NA}
  if(pt_var == 0){pt_var = NA}
  ###function for mean estimation: 
  get_logOR = function(pt, pc){
    logOR = log(pt)-log(1-pt)+log(1-pc)-log(pc)
    return(logOR)
  }
  
  ###function for variance estimation: 
  var_logOR = function(ptrt_est, pctrl_est, ptrt_var, pctrl_var){
    #####jacobian matrix: 
    jac= c( (1/ptrt_est)+ (1/(1-ptrt_est)), -(1/pctrl_est)- (1/(1-pctrl_est)) )
    
    var.terms = matrix(c(ptrt_var, 0, 0, pctrl_var), nrow = 2, ncol = 2)
    
    #####variance estimate: 
    var = t(jac)%*%var.terms%*%jac
    
    return(var)
  }
  
  logOR_est = get_logOR(pt_est, pc_est)
  
  logOR_var = var_logOR(pt_est, pc_est, pt_var, pc_var)
  
  logOR_ci = c( logOR_est-1.96*sqrt(logOR_var),  logOR_est+1.96*sqrt(logOR_var) )
  
  logOR_res = list(est = logOR_est, var = c(logOR_var), ci = logOR_ci )
  
  res.list = list(pc = pc_res, 
                  pt = pt_res, 
                  logOR = logOR_res, 
                  all_pscl_res = rst_cl_rct, 
                  ps_res = disc_ps)
...
SS_MIX_MEM = function(dat, M1, M2, nburnin1, nburnin2, prior.text, borrow_text){
  library(dplyr); library(tidyr)
  ####get dat set up for functions: ######
  RCT <- dat[[1]]; 
  PS_rct <- RCT$logitPS
  treated <- filter(RCT, Trt == 1); n_trt_t  = nrow(treated); r_trt_t = sum(treated$Y)
  untreated <- filter(RCT, Trt == 0); n_ctrl_t  =  nrow(untreated); r_ctrl_t = sum(untreated$Y)
  ####treated external:
  ext <- dat[[2]]
  trt_ext <- filter(ext, Trt == 1);  
  ps_trt_ext <- trt_ext$logitPS; 
  y_trt_ext <- trt_ext$Y
  untrt_ext <- filter(ext, Trt == 0);
  ps_ctrl_ext <- untrt_ext$logitPS;
  y_ctrl_ext <- untrt_ext$Y
  ...
  if(borrow_text == "both"){
    ...
    SS_trt <- SS_MIX(PS_rct, ps_trt_ext,  y_trt_ext, Mall1,
                     0.00001, .001,.001, 0.00001, .001,  .001,  1, 1)
  ...
  if(borrow_text == "control"){
    ...
    n_in_ctrl = mean(SS_ctrl[,2]); y_in_ctrl = mean(SS_ctrl[,1])
    n_in_trt = mean(SS_trt[,2]); y_in_trt = mean(SS_trt[,1])
    #Zs = SS_ctrl_all$Zvec[-c(1:nburnin1), ]
    #draws_ps_IN = do.call(rbind, lapply(1:M1, function(x) data.frame( Z = Zs[x, ], PS = ps_ctrl_ext) %>% filter(Z == 1)) )
    #draws_ps_IN = draws_ps_IN$PS
    ####if prior.text = "capped" 
    if(prior.text == "FB") {pip_ctrl <- 0.5; pip_trt <- 0}
    if(prior.text == "capped"){
      pip_ctrl <- mean(do.call(rbind, 
  ...
  if(borrow_text == "treated"){
    SS_trt_all <- SS_MIX(PS_rct, ps_trt_ext,  y_trt_ext, Mall1,
                         0.00001, .001,.001, 0.00001, .001,  .001,  1, 1)
    
    SS_trt <- cbind(SS_trt_all$yobsIN, SS_trt_all$nobsIN)[-c(1:nburnin1), ]
    SS_ctrl <- cbind( rep(0, M1), rep(0, M1) )
    mix_ctrl = NA; mix_trt = mean(SS_trt_all$w[-c(1:nburnin1)])
    n_in_ctrl = mean(SS_ctrl[,2]); y_in_ctrl = mean(SS_ctrl[,1])
    n_in_trt = mean(SS_trt[,2]); y_in_trt = mean(SS_trt[,1])
    ####if prior.text = "capped" 
    if(prior.text == "FB") {pip_ctrl <- pip_trt <- 0.5}
  ...
  weights_trt = do.call(rbind, lapply(1:M1, function(x) {list_draws[[x]]$weights_ctrl[2]}) ) 
